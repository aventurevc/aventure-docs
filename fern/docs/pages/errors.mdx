...
and appear on every error. `type` is a URI drawn from a fixed set — values such
as `https://api.aventure.vc/errors/access-denied`, `.../rate-limited`, and
`.../subscription-required` — so it is the most reliable field to branch on when
the status alone does not say what to do. Everything else is an aVenture
extension, present only when it applies:
...
| `402` | The operation needs an entitlement this user does not have | Plain-English search needs a current Pro subscription or trial — check [Settings → Subscription](https://aventure.vc/settings/subscription); a service integration holding a client secret reaches it without one |
