...
the `slug` and the `core.id` UUID the other commands take:

```bash
aventure entities lookup --name Ramp
```

`--url-domain pulley.com` resolves by website instead. A domain is often shared
by a company and the product records under it, and then the answer is `409`;
resolve by `--name`, which stays unambiguous.
