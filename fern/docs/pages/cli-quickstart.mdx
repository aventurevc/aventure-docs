...
`--url-domain ramp.com` resolves by website instead. When more than one record
owns the domain — a company and one of its products, for example — the answer
is `409`; resolve by `--name` instead.
